  ...
  return `
    <div class="employee-card" data-employee-block>
      ...
      <div class="form-grid">
        ...
        <div class="field"><label>Saida</label><input name="end" type="time" required></div>
      </div>
    </div>
  `;
}
  ...
  return `
    ...
    <section class="card">
      <form class="form-grid" data-form="report">
        <div class="field"><label>Tipo</label><select name="reportType"><option value="summary">Contabilizacao de horas</option><option value="complete">Autorizacao completa</option></select></div>
        <div class="field"><label>Periodo</label><select name="period"><option value="day">Data</option><option value="month">Mes</option><option value="year">Ano</option></select></div>
...
function readEmployeeBlock(block, dayType, retroactive, submitType) {
  ...
  if (calc.error && submitType !== "draft") return { error: `${employeeName}: ${calc.error}` };
  return {
    id: uid(),
    employeeName,
    functionName,
...
function calculateHours({ dayType, start, breakStart, breakEnd, end }) {
  ...
  if (dayType === "workday") {
    ...
    if (!end) return { error: "Informe a saida." };
    if (endMin <= limit) return { error: `Dia util so permite extra depois de ${data.settings.workdayLimit}.` };
    const minutes50 = endMin - limit;
    return { minutes50, minutes100: 0 };
  }
  if (!start || !breakStart || !breakEnd || !end) return { error: "Informe todas as batidas." };
  if ((dayType === "saturday" || dayType === "holiday") && endMin > toMin(data.settings.weekendLimit)) {
    return { error: `Sabado/feriado so permite ate ${data.settings.weekendLimit}.` };
  }
  const lunch = toMin(breakEnd) - toMin(breakStart);
  const minLunch = Number(data.settings.lunchMinutes) - Number(data.settings.lunchTolerance);
  if (lunch < minLunch) return { error: `Almoco precisa ter ao menos ${minLunch} minutos.` };
  const worked = endMin - toMin(start) - lunch;
  if (worked <= 0) return { error: "Horarios invalidos." };
...
function generateReport(fd) {
  ...
  if (fd.get("format") === "excel") {
    downloadCsv(items, `relatorio-facilita-${Date.now()}.xls`);
    return;
  }
...
function filterItems(fd) {
  ...
  return allItems().filter((item) => {
    if (period === "day") return item.date === date;
    if (period === "month") return item.date.startsWith(month);
